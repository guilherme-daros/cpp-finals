...
int getEventTypeMenu() {
    ...
    while (sel == -1) {
        ...
        if (!(std::find(valid_input.begin(), valid_input.end(), sel) != valid_input.end())) {
            sel = -1;
        }
    }
    return sel;
...
std::vector<EventBase*> getTimedSliceMenu(std::vector<EventBase*> vector_to_slice, time_t beginTime,
                                          time_t endTime) {
